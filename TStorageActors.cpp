...
TStorageActors::TStorageActors():isRun(false),_thread(),_isUpdateTimer(false),_lockTimer(),_timeAndTask(),cv()
{
...
TStorageActors::~TStorageActors(){ if (isRun) {  isRun=false; _thread.join();}}
...
void TStorageActors::execute(const uint TimeOut)//выбираем сообщение из очереди и вызываем соответствующий функтор
{
    ...
    try{
        ...
        if (_mapFunctorsOnMessage.end()==_mapFunctorsOnMessage.find(message->getMessage())){_mapFunctorsOnMessage[UncnowCommand](message, this->getThis() );}
...
void TStorageActors::executeTask()
{
    ...
    while(isRun)
    {
        try
        {
            ...
            if(cv.wait_until(lk, timer, [this](){return _isUpdateTimer==true;})) //часты ложные срабатывания
            {
            ...
            for (auto& i:_timeAndTask)
            {
                ...
                //std::cout<<"for"<<"||"<<(i.second.first<=now)<<"||"<<(i.second.first-now).count()<<std::endl;
